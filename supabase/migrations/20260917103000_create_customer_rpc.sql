...
-- Database Migration: 20260917103000_create_customer_rpc.sql
-- Description: Atomic SECURITY DEFINER customer registration and scheme enrollment RPC.
--              Verified against 001_initial_schema.sql and 002_rls_security.sql.
-- =============================================================================
...
BEGIN
    ...
    IF pg_catalog.length(pg_catalog.trim(p_full_name)) < 2 THEN
    ...
    IF p_alternate_phone IS NOT NULL THEN
        v_clean_alt_phone := pg_catalog.nullif(pg_catalog.regexp_replace(p_alternate_phone, '\D', '', 'g'), '');
    ELSE
    ...
    INSERT INTO public.customers (
        ...
        v_clean_phone,
        pg_catalog.trim(p_full_name),
        pg_catalog.nullif(pg_catalog.trim(p_address), ''),
        pg_catalog.coalesce(pg_catalog.nullif(pg_catalog.trim(p_city), ''), 'Dindigul'),
        pg_catalog.nullif(pg_catalog.trim(p_pincode), ''),
        v_clean_alt_phone,
        pg_catalog.nullif(pg_catalog.trim(p_nominee_name), ''),
        pg_catalog.nullif(pg_catalog.trim(p_nominee_relationship), ''),
        pg_catalog.nullif(pg_catalog.trim(p_notes), ''),
        v_admin_id,
        v_now,
        v_now
    );
    ...
    PERFORM private.log_audit(
        ...
        jsonb_build_object(
            ...
            'full_name', pg_catalog.trim(p_full_name)
        ),
    ...
    IF p_enroll_scheme IS TRUE THEN
        v_start_date := pg_catalog.date_trunc('month', pg_catalog.coalesce(p_start_month, v_now::date))::DATE;
...
REVOKE ALL ON FUNCTION public.create_customer_with_scheme(VARCHAR, VARCHAR, TEXT, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, TEXT, BOOLEAN, DATE) FROM PUBLIC;

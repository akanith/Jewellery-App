import { ShopInfo } from '@/types/shop';

export const OFFICIAL_SHOP_INFO: ShopInfo = {
  name: "Ramya's Jeweller",
  tagline: 'Ramyas Jeweller Savings Scheme Partner',
  category: 'Jewellery Store',
  address: '91, Main Road, Begambur',
  city: 'Dindigul',
  state: 'Tamil Nadu',
  pincode: '624001',
  country: 'India',
  phone: '+91 98421 43307',
  whatsappPhone: '919842143307',
  email: 'contact@ramyasjeweller.com',
  openingTime: '09:30',
  closingTime: '22:00',
  workingDays: 'Monday-Sunday',
  googleMapsUrl:
    'https://www.google.com/maps/search/?api=1&query=RAMYA%27S%20JEWELLER%2091%2C%20Main%20Road%2C%20Dindigul%2C%20Begambur%2C%20Tamil%20Nadu%20624001%2C%20India',
};

export const OFFICIAL_SCHEME_NAME = 'Ramyas Jeweller Savings Scheme';

